...
def seq2seq_loss_function(real, pred, padding_mask):
    ...
    for t in range(real.shape[1]):
        ...
        mask = tf.cast(padding_mask[:, t], loss_.dtype)
...
def pgn_log_loss_function(real, final_dists, padding_mask):
    ...
    for dec_step, dist in enumerate(final_dists):
        ...
        # losses = tf.negative(tf.math.log(gold_probs),name = None)#(-tf.math.log(gold_probs))
...
def _mask_and_avg(values, padding_mask):
    ...
    padding_mask = tf.cast(padding_mask, values[0].dtype)
